...
# 추가 구현해야하는 것들,,, 주 단위 검색( ex) 이번주 경기일정 ), 월 단위 검색( ex)이번달 경기일정 ),
# 일정 조회 함수에서 진행 중인 경기일 때 코드 추가 (현재는 진행 중 경기도 끝난 걸로 나옴),
# 어제, 내일, 모래 등 날짜 datetime을 str로 정규화하는 것,,,
# 함수 문 안에 result가 왜 10월만 뜰까,,

# 오늘 날짜
input = dt.date.today() - timedelta(30)
ans = ""
# 일정 조회 함수
def Search(search):
    global ans
    ...
    for dataTb in soupData:
        for data in dataTb:
            ...
            if (len(dateValue.split(" ")[0].split(".")[1]) == 1):
                # 날짜 정규화
                dateValue = dateValue.split(" ")[0].split(".")[0] + ".0" + dateValue.split(" ")[0].split(".")[1] + " " + \
                            dateValue.split(" ")[1]
